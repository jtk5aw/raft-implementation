...
use pingora::{
    ...
    services::Service as OtherService,
...
fn main() -> std::io::Result<()> {
    ...
    let cert_path = &base_dir.join(CERTS).join(RISDB_PEM);
    let cert_path = cert_path.to_str().unwrap();
    let key_path = &base_dir.join(CERTS).join(RISDB_EC);
    ...
    let upstreams = LoadBalancer::try_from_iter(["[::1]:3000", "[::1]:3001"]).unwrap();
    ...
    let services: Vec<Box<dyn OtherService>> = vec![Box::new(lb)];
